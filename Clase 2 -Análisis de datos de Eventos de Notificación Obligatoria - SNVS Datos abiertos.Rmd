---
title: Clase 2 - Análisis de datos de Eventos de Notificación Obligatoria - SNVS Datos
  abiertos
author: "MGAIE"
date: "`r Sys.Date()`"
...
Esta función no funciona porque no está creando la carpeta de destino revisamos y los archivos
```{r}
...
```
Vemos que los dataset tienen un error en como mencionan la variable año en la col 5
...
Se procede a agrupar a nivel de provincia_nombre,ano, semanas_epidemiologicas ya que son las variables con las que voy a contruir mi serie de tiempo.
...
```{r}
grafico =
  # 1. llamar a highchart
  highchart() %>% 
  #2. Define el tipo de grafico y la funcion para hacer zoom a algun eje
  hc_chart(type = "line",
           zoomType = 'xy') %>% 
  #3. Define titulo
  hc_title(text = "Notificaciones de ETI por SEPI") %>% 
  #4. categorías para los ejes de x
  hc_xAxis(categories = unique(ts_base$semana))%>% 
...
#aca se hace un loop donde se itera por las provincias seleccionadas usando la fn de hc_add_series del paquete highcharter. se esta manera no hay que armar un hc_add_series para cada una de las series (amé)
...
```{r}
sifilis <-  read.csv("http://datos.salud.gob.ar/dataset/2ccd96b6-8cfc-4cd2-a685-6d2298a97b60/resource/de78a103-81c5-4643-a789-0a80a9c556a1/download/tasa-sifilis-por-100-mil-habitantes-sexo-jurisdiccion-2018-2020-argentina_1.csv", encoding = "latin1")
...
# buscar alternativas oficiales a esto
...
mapa_pais <- ggplot(mapaProv_sf_tasa %>% 
                      filter(ano>=2013)) + ## se queda con 2013 en caso que tenga una serie de tiempo  
  ...
  coord_sf(crs = "+proj=tmerc +lat_0=-90 +lon_0=-66 +k=1 +x_0=3500000 +y_0=0 +ellps=GRS80 +towgs84=0,0,0,0,0,0,0 +units=m +no_defs")+ # y esto que sería????
  theme_minimal()+
  theme(
